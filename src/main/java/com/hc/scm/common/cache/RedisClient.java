 ...
 * 
 */
package com.hc.scm.common.cache;
...
public class RedisClient {
	 ...
	 * @param conn       
	 */
	public void returnConnection(ShardedJedis jedis) {
		if (null != jedis) {
			try {
	 ...
	 * @param jedis
	 */
	public void returnBorkenConnection(ShardedJedis jedis) {
		if (null != jedis) {
			jedisPool.returnBrokenResource(jedis);
		}
	}
	...
	public long expire(String key, int seconds) {
		ShardedJedis conn = getConnection();
		if (conn == null) {
			return -1;
		}
		try {
	...
	public long delete(String key) {
		...
		try {
			...
			if(logger.isDebugEnabled()){
				logger.error("delete rediscatch key:"+key);
			}
			returnConnection(conn);
			return result;
		} catch (Exception e) {
	...
	public Object get(Object key) {
		...
		if (data != null) {
			if(logger.isDebugEnabled()){
				logger.error("get rediscatch key:"+key+" usedtime:"+(System.currentTimeMillis()-starttime));
			}
